...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.canada_nfdb.wildfire_statistics \
       --csv burnt.csv \
       > canada_nfdb_wildfire_statistics.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af canada_nfdb.wildfire_statistics; tail -n 20 GisFIRE/canada_nfdb_wildfire_statistics.log'
