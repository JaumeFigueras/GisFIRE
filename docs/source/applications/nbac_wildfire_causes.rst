...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.canada_nbac.wildfire_causes \
       --csv causes.csv \
       > canada_nbac_wildfire_causes.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af canada_nbac.wildfire_causes; tail -n 20 GisFIRE/canada_nbac_wildfire_causes.log'
