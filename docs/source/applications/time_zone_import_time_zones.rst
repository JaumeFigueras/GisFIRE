...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.time_zones.timezone_boundary_builder.import_time_zones \
       -s timezones-with-oceans.shapefile.zip \
       > timezone_boundary_builder_import_time_zones.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f timezone_boundary_builder_import_time_zones.log    # Ctrl-C stops tail, not the run
   pgrep -af timezone_boundary_builder.import_time_zones    # still running?
   grep -E 'ERROR|WARNING' timezone_boundary_builder_import_time_zones.log
