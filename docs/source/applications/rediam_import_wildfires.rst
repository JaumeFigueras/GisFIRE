...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.wildfires.andalusia_rediam.import_wildfires \
       -d /path/to/andalusia/InfGeografica/InfVectorial/Shapes/ \
       > andalusia_rediam_import_wildfires.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f andalusia_rediam_import_wildfires.log    # Ctrl-C stops tail, not the run
   pgrep -af andalusia_rediam.import_wildfires    # still running?
   grep -E 'ERROR|WARNING' andalusia_rediam_import_wildfires.log
