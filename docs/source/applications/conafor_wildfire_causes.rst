...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.mexico_conafor.wildfire_causes \
       --csv causes.csv \
       > mexico_conafor_wildfire_causes.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af mexico_conafor.wildfire_causes; tail -n 20 GisFIRE/mexico_conafor_wildfire_causes.log'
