...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.gfa.wildfire_statistics \
       --csv burnt.csv \
       > gfa_wildfire_statistics.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af gfa.wildfire_statistics; tail -n 20 GisFIRE/gfa_wildfire_statistics.log'
