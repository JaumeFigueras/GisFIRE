...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.spain_egif.wildfire_causes \
       --csv causes.csv \
       > spain_egif_wildfire_causes.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af spain_egif.wildfire_causes; tail -n 20 GisFIRE/spain_egif_wildfire_causes.log'
