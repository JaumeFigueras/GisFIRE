...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.guatemala_inab.wildfire_classification \
       --csv location.csv \
       --docx location.docx \
       > guatemala_inab_wildfire_classification.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af guatemala_inab.wildfire_classification; tail -n 20 GisFIRE/guatemala_inab_wildfire_classification.log'
   scp user@server:GisFIRE/location.csv user@server:GisFIRE/location.docx .
