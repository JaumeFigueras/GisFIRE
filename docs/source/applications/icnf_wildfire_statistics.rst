...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.portugal_icnf.wildfire_statistics \
       --csv burnt.csv \
       > portugal_icnf_wildfire_statistics.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af portugal_icnf.wildfire_statistics; tail -n 20 GisFIRE/portugal_icnf_wildfire_statistics.log'
