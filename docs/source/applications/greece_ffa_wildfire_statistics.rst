...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.greece_ffa.wildfire_statistics \
       --csv burnt.csv \
       --docx burnt.docx \
       > greece_ffa_wildfire_statistics.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af greece_ffa.wildfire_statistics; tail -n 20 GisFIRE/greece_ffa_wildfire_statistics.log'
   scp user@server:GisFIRE/burnt.csv user@server:GisFIRE/burnt.docx .
