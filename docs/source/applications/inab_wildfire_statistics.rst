...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.guatemala_inab.wildfire_statistics \
       --csv guatemala.csv \
       --docx guatemala.docx \
       > guatemala_inab_wildfire_statistics.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af guatemala_inab.wildfire_statistics; tail -n 20 GisFIRE/guatemala_inab_wildfire_statistics.log'
   scp user@server:GisFIRE/guatemala.csv user@server:GisFIRE/guatemala.docx .
