...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.admin_boundaries.caop.import_admin_boundaries \
       -d /path/to/caop \
       > caop_import_admin_boundaries.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f caop_import_admin_boundaries.log    # Ctrl-C stops tail, not the run
   pgrep -af caop.import_admin_boundaries    # still running?
   grep -E 'ERROR|WARNING' caop_import_admin_boundaries.log
