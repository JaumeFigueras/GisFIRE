...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.andalusia_rediam.wildfire_statistics \
       --csv burnt.csv \
       > andalusia_rediam_wildfire_statistics.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af andalusia_rediam.wildfire_statistics; tail -n 20 GisFIRE/andalusia_rediam_wildfire_statistics.log'
