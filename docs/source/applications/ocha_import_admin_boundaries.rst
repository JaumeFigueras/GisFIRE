...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.admin_boundaries.ocha.import_admin_boundaries \
       -g adm0_polygons.gpkg \
       > ocha_import_admin_boundaries.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f ocha_import_admin_boundaries.log    # Ctrl-C stops tail, not the run
   pgrep -af ocha.import_admin_boundaries    # still running?
   grep -E 'ERROR|WARNING' ocha_import_admin_boundaries.log
