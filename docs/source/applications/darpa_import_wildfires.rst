...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.wildfires.catalonia_darpa.import_wildfires \
       -d /path/to/catalunya/ \
       > catalonia_darpa_import_wildfires.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f catalonia_darpa_import_wildfires.log    # Ctrl-C stops tail, not the run
   pgrep -af catalonia_darpa.import_wildfires    # still running?
   grep -E 'ERROR|WARNING' catalonia_darpa_import_wildfires.log
