...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.statistics.wildfires.spain_egif.wildfire_statistics \
       --csv burnt.csv \
       > spain_egif_wildfire_statistics.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   ssh user@server 'pgrep -af spain_egif.wildfire_statistics; tail -n 20 GisFIRE/spain_egif_wildfire_statistics.log'
