...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.wildfires.canada_nfdb.import_wildfires \
       -s /path/to/NFDB_point_shp.zip \
       > canada_nfdb_import_wildfires.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f canada_nfdb_import_wildfires.log    # Ctrl-C stops tail, not the run
   pgrep -af canada_nfdb.import_wildfires    # still running?
   grep -E 'ERROR|WARNING' canada_nfdb_import_wildfires.log
