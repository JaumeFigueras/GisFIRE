...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.bindings.wildfires.canada_nbac.bind_nfdb_wildfires \
       > canada_nbac_bind_nfdb_wildfires.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f canada_nbac_bind_nfdb_wildfires.log    # Ctrl-C stops tail, not the run
   pgrep -af canada_nbac.bind_nfdb_wildfires    # still running?
   grep -E 'ERROR|WARNING' canada_nbac_bind_nfdb_wildfires.log
