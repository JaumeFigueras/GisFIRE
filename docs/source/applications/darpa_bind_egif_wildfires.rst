...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.bindings.wildfires.catalonia_darpa.bind_egif_wildfires \
       > catalonia_darpa_bind_egif_wildfires.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f catalonia_darpa_bind_egif_wildfires.log    # Ctrl-C stops tail, not the run
   pgrep -af catalonia_darpa.bind_egif_wildfires    # still running?
   grep -E 'ERROR|WARNING' catalonia_darpa_bind_egif_wildfires.log
