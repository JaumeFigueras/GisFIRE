...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.wildfires.chile_conaf_magnitud.import_wildfires \
       -d perimetres/ \
       > chile_conaf_magnitud_import_wildfires.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f chile_conaf_magnitud_import_wildfires.log    # Ctrl-C stops tail, not the run
   pgrep -af chile_conaf_magnitud.import_wildfires    # still running?
   grep -E 'ERROR|WARNING' chile_conaf_magnitud_import_wildfires.log
