...
.. code-block:: bash
   ...
   ssh user@server
   cd ~/GisFIRE
   nohup .venv/bin/python -u -m src.apps.imports.wildfires.portugal_icnf.import_wildfires \
       -d /path/to/portugal/ \
       > portugal_icnf_import_wildfires.log 2>&1 < /dev/null &
   exit
...
.. code-block:: bash
   ...
   cd ~/GisFIRE
   tail -f portugal_icnf_import_wildfires.log    # Ctrl-C stops tail, not the run
   pgrep -af portugal_icnf.import_wildfires    # still running?
   grep -E 'ERROR|WARNING' portugal_icnf_import_wildfires.log
