...
def test_user_access_init_01() -> None:
    ...
    assert user_access.result == 500
    assert user_access.user_id is None
    user_access = UserAccess(ip=IPv4Address('123.45.67.89'))
    assert user_access.ip == IPv4Address('123.45.67.89')
    assert user_access.url is None
    assert user_access.method is None
    assert user_access.params is None
    assert user_access.result == 500
    ...
    assert user_access.result == 500
    assert user_access.user_id is None
    user_access = UserAccess(result=200)
    assert user_access.ip is None
    assert user_access.url is None
    assert user_access.method is None
    assert user_access.params is None
    ...
    user_access = UserAccess(ip=IPv4Address('159.25.36.214'), url='/api/test', method=HttpMethods.GET, params={'date': '2024-01-01'}, result=404, user=1)
    assert user_access.ip == IPv4Address('159.25.36.214')
