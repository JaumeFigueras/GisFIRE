...
def process_ignitions(params):
    ...
    try:
        ...
        for id, ignition_dct in ignitions.items():
            ...
            stmt = stmt.where(MeteocatLightning._date_time <= ignition.start_date_time + datetime.timedelta(days=1))
        ...
        for id, ignition_dct in ignitions.items():
            ...
            for lightning_dct in ignition_dct['lightnings']:
                ...
                t = ((ignition.start_date_time + datetime.timedelta(days=1)) - lightning.date_time).total_seconds() / 3600
                a = (1 - (t / (time_back + 24))) * (1 - (distance / radius))
                lightning_dct['A'] = a
            mult = 1
            suma = 0
            for lightning_dct in ignition_dct['lightnings']:
        ...
        for id, ignition_dct in ignitions.items():
            ...
            experiment.name = 'assign_lightning_to_wildfire_+1'
            experiment.params = {
                'ignition_id': str(id),
                'year': str(year),
                'radius': str(radius),
